...
class PrintTimestep(hoomd.custom.Action):
    ...
    def act(self, timestep):
        global old_time, old_step, meas_period, initialStep
        ...
        if( (current_time - old_time) > 10 ):
            ...
            timestep_elapsed = timestep - initialStep

            TPS = timestep_elapsed/(current_time)

            ETA = nsteps_rem/TPS  - current_time
...
initialStep = simulation.timestep

nsteps_rem = nsteps - initialStep
...
time_start = time.time()
old_time = 0
old_step = np.floor(initialStep/meas_period*10)*meas_period/10
