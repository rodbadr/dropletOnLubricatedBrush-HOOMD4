...
sys.path.insert(0, "../customLoggers") # Import custom loggers from the specified path
from stressRelaxationFunctionLogger import separatePressureTensor
...
# read the parameter index from the command line
parser.add_argument('paramInd', type=int, help='Parameter line to read')

# add an optional argument to control whether to calculate the correlation function (default is False)
parser.add_argument('--correlate', action='store_true', help='Calculate correlation function')

# add an optional argument to control whether to log the pressure tensor (default is False)
parser.add_argument('--logPressure', action='store_true', help='Log pressure tensor')

args = parser.parse_args()


# read simulation parameter file

paramInd = args.paramInd


paramSpace = np.genfromtxt('ParameterSpaceOliBrushSetup.csv', delimiter=',')

params = paramSpace[paramInd-1,:]

fileroot_out = 'oliBrush_NPT' # output file name root for the simulations
...
if args.logPressure: # Check if the --logPressure argument is provided

    # trigger for pressure tensor logging
    # logging 10x more frequently than the trajectory
    presLog_trig = hoomd.trigger.Periodic(period=int(meas_period/10), phase=0)

    # declare the logger for pressure tensor
    pressureTensor_logger = hoomd.logging.Logger(categories=["scalar", "sequence"])

    # add the timestep and pressure tensor to the logger
    pressureTensor_logger.add(simulation, quantities=["timestep"])
    pressureTensor_logger.add(thermodynamic_properties, quantities=["pressure_tensor"])

    # declare the writer for pressure tensor and append it to the simulation writers
    hdf5_writer = hoomd.write.HDF5Log(trigger=presLog_trig, filename=f"Data/{fileroot_out}PressureTensor{sout}.h5", mode='w', logger=pressureTensor_logger)
    simulation.operations.writers.append(hdf5_writer)


if args.correlate:  # Check if the --correlate argument is provided
    ...
    # and will allow us to log the separated components
    presTensor_separated = separatePressureTensor()
    
    # Create a custom writer for the separated pressure tensor components
    ...
    pres_writer = hoomd.write.CustomWriter(action=presTensor_separated, trigger=hoomd.trigger.Periodic(1))
    simulation.operations.writers.append(pres_writer)
    ...
    # add the pressure tensor components to the logger.
    stressRel_logger.add(presTensor_separated, quantities=quantities)

    # Create the autocorrelator action object with the specified quantities and logger and output filename
    correlator_action = autocorrelate(quantities=quantities, logger=stressRel_logger, filename=f"Data/correlationFunctions{sout}.csv")

    # Create a custom writer for the correlator action.
    # Although the action itself does not write anything when eval_period=0,
    # this will make sure that the act() method is called at each trigger timestep
    # the final output will be written to the file when write_to_file is called below
    correlator_writer = hoomd.write.CustomWriter(action=correlator_action, trigger=hoomd.trigger.Periodic(1))
    simulation.operations.writers.append(correlator_writer)
...
if args.correlate:  # Check if the --correlate argument is provided
    ...
    correlator_action.write_to_file(simulation.timestep)
