...
fileroot_in = 'oliBrush' # output file name root for the simulations
...
nsteps = int(4e5) # number of simulation steps
i = i+1
...
try:
    ...
    print("No initial file found. Trying to read from NVT simulation.")

    tempFname = "initialConditions/" + fileroot_in + sout + ".gsd"

    tempSnap = gsd.hoomd.open(name=tempFname,mode='r')
    snapBrush = tempSnap[0]

    Lx = brushDist*Nbrush1
    Ly = brushDist*Nbrush2

    Lz = snapBrush.configuration.box[2];

    positionsBrush = snapBrush.particles.position
    bondsBrush = snapBrush.bonds.group
    typeID_Brush = snapBrush.particles.typeid

    positionsBrush[:,2] = (positionsBrush[:,2] - positionsBrush[0,2] + Lz/2)%Lz - Lz/2 # put grafted monomers at the center of the box
...
npt = hoomd.md.methods.ConstantPressure(filter=mobile, S=1e-1, tauS=1, couple="none", thermostat=None, box_dof=[False, False, True, False, False, False])
