...
class PrintTimestep(hoomd.custom.Action):
    ...
    def act(self, timestep):
        global old_time, old_step, meas_period, init_tstep
        ...
        if( (current_time - old_time) > 10 ):
            ...
            TPS = (timestep - init_tstep)/(current_time)
            ...
            print("Elapsed time {:02d}:{:02d}:{:02d}".format(tH,tM,tS) + " | Step {:d}/{:d} | TPS={:.6g} ".format(timestep,init_tstep+nsteps,TPS) + " | ETA {:02d}:{:02d}:{:02d}".format(tH2,tM2,tS2), flush=True )
...
init_tstep = simulation.timestep
