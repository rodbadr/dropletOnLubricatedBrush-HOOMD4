...
paramSpace = np.genfromtxt('ParameterSpaceContactingSlabs.csv', delimiter=',')
...
i=i+1
oliThick1 = float(params[i]); #thickness of polymer slab
i=i+1

oliLen2 = int(params[i]); # number of monomers per oligomer chain
i=i+1
oliThick2 = float(params[i]); #thickness of polymer slab
i=i+1
...
comZslab1 = computeCOM(snap1.particles.position[:,2] + Lz1/2, Lz1) - Lz1/2 # center of mass of the slab
...
for chain in range(Noli1):
    ...
    comzArr1[chain] = computeCOM(snap1.particles.position[ind1:ind2,2] + Lz1/2, Lz1) - Lz1/2


mask = ( comzArr1 >  bottom1 )
mask2 = ( comzArr1 <  (bottom1 + oliThick1) )
mask3 = mask & mask2

mask3 = np.repeat(mask3, oliLen1) # repeat the mask to apply it to monomers instead of chains


Ntot1 = int(mask3.sum()) # number of particles remaining
Noli1 = int(Ntot1/oliLen1) # number of chains remaining

positions1 = np.zeros((Ntot1,3))
positions1[:] = snap1.particles.position[mask3] # positions of selected particles
...
comZslab2 = computeCOM(snap2.particles.position[:,2] + Lz2/2, Lz2) - Lz2/2 # center of mass of the slab
...
for chain in range(Noli2):
    ...
    comzArr2[chain] = computeCOM(snap2.particles.position[ind1:ind2,2] + Lz2/2, Lz2) - Lz2/2


mask = ( comzArr2 <  top2 )
mask2 = ( comzArr2 >  (top2 - oliThick2) )
mask3 = mask & mask2

mask3 = np.repeat(mask3, oliLen2) # repeat the mask to apply it to monomers instead of chains


Ntot2 = int(mask3.sum()) # number of particles remaining
Noli2 = int(Ntot2/oliLen2) # number of chains remaining

positions2 = np.zeros((Ntot2,3))
positions2[:] = snap2.particles.position[mask3] # positions of selected particles
...
positions1[:,2] = positions1[:,2] - (bottom1 + oliThick1)
positions2[:,2] = positions2[:,2] - (top2 - oliThick2)
...
comZslab = computeCOM(positions[:,2] + Lz/2,Lz) - Lz/2 # center of mass of the slab
...
sout = '_' + strFormat.format(oliLen1) + '_' + strFormat.format(oliThick1) + '_' + strFormat.format(oliLen2) + '_' + strFormat.format(oliThick2) + '_' + strFormat.format(-A11) + '_' + strFormat.format(-A22) + '_' + strFormat.format(-A12) + '_' + strFormat.format(B) + '_' + strFormat.format(SimID)
...
### create a semi-permeable wall that allows only type A particles through
### to play the role of the vapor phase


WallPos = Lz/2 - 2*sig_b
...
wall_lj = hoomd.md.external.wall.LJ(walls=walls)
wall_lj.params['A'] = {"sigma": sig_b, "epsilon": eps_b, "r_cut": sig_b*2.0**(1.0/6.0)}
wall_lj.params['B'] = {"sigma": sig_b, "epsilon": eps_b, "r_cut": sig_b*2.0**(1.0/6.0)}
