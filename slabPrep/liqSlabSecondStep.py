...
t = gsd.hoomd.open(name="initialConditions/" + fileroot_out + "Full" + sout + ".gsd",mode='r')
tempSnap = t[0]
...
Ntot = tempSnap.particles.N


positions = np.zeros((Ntot,3))
positions[:] = tempSnap.particles.position[:]


if(oliLen > 1):

    bond_pairs = np.zeros((NbondOli,2))
    bond_pairs[0:NbondOli] = tempSnap.bonds.group[0:NbondOli]



# take care of bonds crossing the periodic boundary in z
...
for i in range(Noli):
    ...
    if(shift == 1):
        ...
        tempz[:] = positions[i*oliLen:(i+1)*oliLen,2]
        tempz2 = np.zeros(oliLen)
        tempz2[:] = tempSnap.particles.position[i*oliLen:(i+1)*oliLen,2]
        tempz[tempz2<0] = tempz[tempz2<0] + Lzin
        positions[i*oliLen:(i+1)*oliLen,2] = tempz[:];
...
initialFname = "initialConditions/" + fileroot_out + "Second" + sout + ".gsd"
